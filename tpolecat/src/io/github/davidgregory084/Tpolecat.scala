...
object Tpolecat {
  private val allScalacOptions = Seq(
    ...
    ScalacOption("-Xlint:nullary-override", isSupported = version => v211 <= version && version < v300),                // Warn when non-nullary `def f()' overrides nullary `def f'.
    ...
    // ScalacOption("-Wself-implicit", isSupported = version => v213 <= version && version < v300),                       // Warn when an implicit resolves to an enclosing self-definition
